...
function create_meeting_recording(request) {
    ...
    mrequest.open("POST", 'https://meet.google.com/$rpc/google.rtc.meetings.v1.MeetingRecordingService/CreateMeetingRecording?', // append ? to avoid our webRequests
    true);
    request.headers.forEach((header) => {
...
function list_meeting_recording_acks(request) {
    ...
    var mrequest = new XMLHttpRequest();
    mrequest.withCredentials = true;
    mrequest.open("POST", 'https://meet.google.com/$rpc/google.rtc.meetings.v1.MeetingRecordingService/ListMeetingRecordingAcks', true);
    request.headers.forEach((header) => {
        mrequest.setRequestHeader(header.name, header.value);
    });
...
function update_meeting_recording(request, command) {
    ...
    var mrequest = new XMLHttpRequest();
    mrequest.withCredentials = true;
    mrequest.open("POST", 'https://meet.google.com/$rpc/google.rtc.meetings.v1.MeetingRecordingService/UpdateMeetingRecording', true);
    request.headers.forEach((header) => {
        mrequest.setRequestHeader(header.name, header.value);
    });
